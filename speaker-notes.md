...
These notes follow the 37 slides in `slides.md` in order. The transition at the end of each section is intended to be spoken as the connection to the next slide.
...
## Slide 2 — Make operating-system mechanisms inspectable

PicoOS is intended as an inspectable implementation of lecture concepts, not just a list of familiar interfaces. From the operating-systems lecture, it brings together parent and child processes, process loading, signals, interrupt vector tables and service routines, software and hardware interrupts, and an allocator that visibly splits and merges heap blocks. Host files are reached only through PicoOS file descriptors; there is no on-device filesystem. From the real-time operating-systems lecture, it connects mutexes, process states, round-robin scheduling, dispatching, exact-child `waitpid`, and wait-queue `sleep` and `wakeup`. The system stays small enough to trace these mechanisms end to end.

**Transition:** The intended hardware makes that small scale concrete and lets us ask whether the available SRAM is sufficient.

## Slide 3 — Two 16-bit SRAMs are enough for PicoOS

The intended setup combines an Alchitry Cu V2 FPGA, two 256K-by-16-bit SRAM chips, and a USB-to-UART adapter. The SRAM chips share the same 18 address and control lines. One supplies data bits zero through fifteen and the other supplies bits sixteen through thirty-one, so together they behave as one 256K-by-32-bit memory. Eighteen address bits select `2^18`, or 262,144, individually addressable 32-bit words. Multiplying by four bytes per word gives exactly 1 MiB.

The current five-word-header-inclusive kernel image uses 37,179 words, leaving 224,965. Dividing by the 8,772-word `cat.bin` image gives 25 copies with 5,665 words remaining. This deliberately conservative comparison even counts the five header words that the loader actually keeps out of SRAM. I would not claim that 25 fully provisioned processes can run simultaneously, because every running process also needs heap and stack space. The calculation instead shows the useful scale: one kernel plus many representative application images fits comfortably in the selected memory.

**Transition:** With the motivation and physical scale established, I can summarize the complete project result.
...
## Slide 6 — The parts I designed and connected
...
## Slide 7 — Loading the kernel from EPROM into SRAM

Execution begins at address zero in EPROM. The bootloader first establishes a valid environment: it sets the code segment to EPROM, places a temporary stack at the top of SRAM, and points the data segment at the bootloader's read-only data. Interrupts are not configured yet, so the bootloader communicates with UART by polling. It sends a request for `kernel/kernel.bin`. The host returns a word count, a five-word layout header, and the encoded program words. The bootloader reads the header but copies only the payload into SRAM. It then changes the code segment, data segment, stack pointer, and base-address frame register to the kernel values and jumps to the compiler-generated kernel start function, which calls `main`.
...
## Slide 8 — Memory constants and UART hardware
...
## Slide 9 — Accessing host files over UART

PicoOS has no disk and no resident filesystem. Instead, it sends bounded escape-sequence requests over UART. A load request returns a binary image; `read-range` and `file-size` support file reads; `write` and `append` select file or standard-stream output; and `pwd`, `ls`, `is-directory`, `mkdir`, `unlink`, and `rmdir` support working directories and directory commands. The host side is implemented by the emulator today; physical hardware would use a companion program connected over USB-to-UART. A program binary begins with five big-endian layout words: code start, data start, heap start, heap size, and stack start. The remaining words are executable code and data, and only that payload is copied into SRAM.
...
## Slide 10 — Kernel initialization
...
## Slide 14 — Handling CPU exceptions
...
## Slide 15 — Handling system calls with `INT 0`
...
## Slide 17 — Timer and UART interrupts
...
## Slide 18 — The process table is a linked list
...
## Slide 21 — `waitpid()` before or after the child exits
...
## Slide 23 — Round-robin scheduling
...
## Slide 24 — Saving and restoring process state
...
## Slide 25 — Memory layout and allocators
...
## Slide 26 — Heap allocation and `free()`
...
## Slide 27 — Kernel, process, and application heaps
...
## Slide 28 — How user programs reach the kernel
...
## Slide 29 — How file descriptors use host files

Every process has a table of eight file descriptors and an absolute working-directory string in its PCB. Descriptors zero, one, and two represent standard input, standard output, and standard error. Input is backed by the UART ring buffer and wait queue, while normal output is sent over UART. Descriptors three through seven can refer to host-backed files and store a path, flags, and a current offset. Relative paths are resolved against the calling process's own directory before file or directory requests cross UART. Children receive copies of descriptors zero through two. The copies have independent offsets because PicoOS does not implement a separate shared open-file object. This is a deliberate simplification, not Unix file-descriptor semantics in full.

**Transition:** Even with this compact descriptor model, the shell can implement normal output redirection using `dup2`.

## Slide 30 — Output redirection with `dup2()`
...
## Slide 31 — Init starts and restarts the shell
...
## Slide 32 — Shell commands and programs

The shell reads a line of up to 80 cells, parses arguments, searches the configured path, expands simple environment variables, and supports output redirection and background jobs. In this example, `export` creates a variable, `echo.bin` expands it and writes to a file, `cat.bin` reads it back, and `count.bin` starts in the background. A trailing ampersand exposes the child's PID through `$!`; `fg`, `bg`, Ctrl+C, and Ctrl+Z provide minimal job control. PicoOS builds 11 application binaries, including file and directory commands, `kill`, and `poweroff`, while 11 built-ins handle operations that must change shell state. Together they exercise nearly every kernel subsystem shown earlier.
...
## Slide 33 — Operating-system mechanisms working together
...
## Slide 34 — How I tested the system

The current test inventory contains 38 cases in three levels: 12 library tests, 17 operating-system feature tests, and 9 shell tests. Library tests focus on functions such as allocation, string handling, formatting, and synchronization. Operating-system tests exercise process arguments and environment, scheduling, signals, file descriptors, shared memory, exceptions, and other kernel features. Shell tests run complete command sessions and compare exact output, including job control, redirection, editing, and errors. The normal `make test` path runs independent OS-backed tests with fresh boots in parallel. The faster `make test-fast` path keeps one operating-system instance for each OS and shell group.

**Transition:** The difference between the normal and fast modes is mainly how they isolate one test from the next.

## Slide 35 — Fresh boots and fast repeated tests
...
## Slide 36 — The finished PicoOS system
...
## Slide 37 — What happens after a key is pressed
